...
namespace BulkyBooks.Areas.Customer.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        //[HttpPost]
        //[ValidateAntiForgeryToken]
        public IActionResult GetProductsOnFilter(string productName)
        {
            IEnumerable<Product> productList = _unitOfWork.Product.GetAll(includeProperties: "Category,CoverType");
            var claimsIdentity = (ClaimsIdentity)User.Identity;
            var claim = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier);
            ...
            {
                productList = productList.Where(x => x.Title.ToLower().Contains(productName.ToLower()) || x.Author.ToLower().Contains(productName.ToLower())).ToList();
            }
            return PartialView("~/Views/Shared/_ProductList.cshtml", productList);
        }
    }
}
