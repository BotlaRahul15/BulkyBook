...
namespace BulkyBooks.Areas.Customer.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public CartController(ILogger<HomeController> logger, IUnitOfWork unitOfWork, IOptions<TwilioSettings> twilionOptions)
        {
            _logger = logger;
        ...
        public IActionResult Index()
        {
            ...
            foreach (var list in ShoppingCartVM.ListCart)
            {
                ...
                if (!string.IsNullOrEmpty(list.Product.Description))
                {
                    list.Product.Description = SD.ConvertToRawHtml(list.Product.Description);
                }
                if (!string.IsNullOrEmpty(list.Product.Description) && list.Product.Description.Length > 100)
                {
        ...
        public IActionResult SummaryPost(string stripeToken)
        {
            ...
            foreach (var product in ShoppingCartVM.ListCart)
            {
                ProductName += product.Product.Title +",";
                BulkyBook.Models.Product p = _unitOfWork.Product.Get(product.ProductId);
                p.Count = p.Count - product.Count;
                _unitOfWork.Product.Update(p);
                _unitOfWork.Save();
            }
            ProductName = ProductName.TrimEnd(',');
            return RedirectToAction("OrderConfirmation", "Cart", new { id = ShoppingCartVM.OrderHeader.Id, produtName = ProductName });

        }

        public IActionResult OrderConfirmation(int id, string produtName)
        {
            ...
            try
            {
                var message = MessageResource.Create(
                    body: "Order Placed on Rahul Bulky Book. Your Order ID:" + id + " "+ "Your Order Name:" + produtName + " Thank You",
